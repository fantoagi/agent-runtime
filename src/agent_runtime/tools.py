...
class ToolResult:
    ...
    def from_value(cls, value: Any) -> ToolResult:
        ...
        return cls(content=json.dumps(value, ensure_ascii=False, default=str), data=value if isinstance(value, dict) else None)
...
class ToolRegistry:
    ...
    def register(self, definition: ToolDefinition, handler: ToolHandler, timeout_seconds: float = 30.0) -> None:
        if definition.name in self._tools:
            raise ValueError(f"Tool {definition.name!r} is already registered.")
        self._tools[definition.name] = RegisteredTool(definition, handler, timeout_seconds)
    ...
    def definitions_for(self, definitions: list[ToolDefinition]) -> list[ToolDefinition]:
    ...
    async def invoke(self, name: str, arguments: dict[str, Any], context: ToolContext) -> ToolResult:
        ...
        try:
            ...
            raise ToolExecutionError(f"Tool {name!r} timed out after {tool.timeout_seconds}s.") from error
...
def validate_input(value: dict[str, Any], schema: dict[str, Any]) -> None:
    ...
    if schema.get("additionalProperties") is False:
        ...
        if unknown:
            raise ToolValidationError(f"Unsupported tool arguments: {', '.join(sorted(unknown))}.")
...
def _validate_value(key: str, value: Any, schema: dict[str, Any]) -> None:
    ...
    if "enum" in schema and value not in schema["enum"]:
        raise ToolValidationError(f"Argument {key!r} must be one of {schema['enum']!r}.")
...
def _calculator(arguments: dict[str, Any], context: ToolContext) -> str:
    ...
    if not expression or any(char not in allowed for char in expression):
        raise ToolValidationError("Calculator accepts only digits, whitespace, and + - * / % ( ).")
    try:
        return str(eval(expression, {"__builtins__": {}}, {}))  # noqa: S307 - grammar is constrained above.
    except Exception as error:
        raise ToolValidationError(f"Invalid arithmetic expression: {error}") from error
...
def _write_text_file(arguments: dict[str, Any], context: ToolContext) -> dict[str, str]:
    path = confined_path(context.workspace_path, arguments["path"])
