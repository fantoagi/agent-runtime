...
- 通过 `AgentRegistry` 注册、发现并校验 `AgentDefinition`。
- 创建、启动、等待和恢复 `AgentRun`，并为每个 Run 注入稳定 `trace_id`。`submit()` 支持 durable idempotency key。
...
- `AgentRegistry`：保存当前进程可委派的 AgentDefinition；同名冲突定义会被拒绝。
